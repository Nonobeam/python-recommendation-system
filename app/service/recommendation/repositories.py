...
from app.exception.cache_exceptions import DemographicDataError, RedisOperationError
...
class RedisDemographicRepository(DemographicRepository):
    ...
    def store(self, entity_id: str, data: Dict[str, Any], expire: int = 3600) -> bool:
        try:
            cache_key = self._get_cache_key(entity_id)
            return cache.set(cache_key, data, expire=expire)
        except RedisOperationError as e:
            api_logger.error(f"Failed to store cache for {entity_id}: {str(e)}")
            return False
...
class DemographicDataSource:
    ...
    def get_by_id(self, entity_id: str) -> Optional[Dict[str, Any]]:
        ...
        if db_data:
            self.redis_repo.store(entity_id, db_data, self.cache_ttl)
