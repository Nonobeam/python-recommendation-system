...
from app.service.recommendation.services import SingleMatchService, BatchMatchService
...
async def calculate_single_match_score(request: SingleMatchRequest = Body(...)):
    ...
    try:
        if not is_valid_uuid(request.brand_id):
            raise HTTPException(
                status_code=400,
                detail=f"Invalid brand_id format: {request.brand_id}"
            )
        
        if not is_valid_uuid(request.mall_id):
            raise HTTPException(
                status_code=400,
                detail=f"Invalid mall_id format: {request.mall_id}"
            )
...
async def calculate_batch_match_scores(request: BatchMatchRequest = Body(...)):
    ...
    try:
        ...
        if not request.mall_ids or len(request.mall_ids) == 0:
            raise HTTPException(
                status_code=400,
                detail="mall_ids array cannot be empty"
            )
        ...
        for mall_id in request.mall_ids:
            if not is_valid_uuid(mall_id):
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid mall_id format in array: {mall_id}"
                )
        
        if len(request.mall_ids) > 100:
            raise HTTPException(
                status_code=400,
                detail="Maximum 100 mall_ids allowed per batch request"
            )
...
async def calculate_paginated_batch_scores(
    ...
    try:
        ...
        if not mall_id_list:
            raise HTTPException(
                status_code=400,
                detail="mall_ids parameter cannot be empty"
            )
        ...
        for mall_id in mall_id_list:
            if not is_valid_uuid(mall_id):
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid mall_id format: {mall_id}"
                )
